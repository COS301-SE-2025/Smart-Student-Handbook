...
export const dynamic = "force-dynamic" // avoid static prerender of this page
...
export default function FriendsPage() {
  ...
  const getFriendRequestsFunc = httpsCallable<{}, { incoming: UserProfile[]; sent: UserProfile[] }>(
    fns,
    "getFriendRequests"
  )
  const acceptFriendRequestFunc = httpsCallable<{ targetUserId: string }, { success: boolean; message: string }>(
    fns,
    "acceptFriendRequest"
  )
  const rejectFriendRequestFunc = httpsCallable<{ targetUserId: string }, { success: boolean; message: string }>(
    fns,
    "rejectFriendRequest"
  )
  const cancelFriendRequestFunc = httpsCallable<{ targetUserId: string }, { success: boolean; message: string }>(
    fns,
    "cancelFriendRequest"
  )
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      {/* Suspense boundary satisfies Next.js requirement for any child using useSearchParams() */}
                  ...
                  {incomingRequests.length === 0 ? (
                    ...
                    <div className="space-y-3">
                      {incomingRequests.map((req) => (
                        <div key={req.uid} className="flex items-center justify-between bg-muted/30 border p-3 rounded-lg">
                          ...
                          <div className="flex gap-2">
                            ...
                            <Button size="sm" variant="outline" onClick={() => handleReject(req.uid)} className="h-8 px-3">
                  ...
                  {sentRequests.length === 0 ? (
                    ...
                    <div className="space-y-3">
                      {sentRequests.map((req) => (
                        <div key={req.uid} className="flex items-center justify-between border p-3 rounded-lg bg-muted/20">
                          <Link href={`/friends/${req.uid}`} className="flex items-center gap-3 flex-1">
                            <Avatar className="h-8 w-8">
                              <AvatarImage src={req.profilePicture || "/placeholder.svg"} />
                              <AvatarFallback className="bg-muted text-foreground text-xs">
