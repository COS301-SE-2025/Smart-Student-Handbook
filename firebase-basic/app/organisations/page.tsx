...
export default function OrganisationsPage() {
  ...
  const getPublicOrgs = useMemo(
    () =>
      httpsCallableFromURL<{}, Org[]>(
        fns,
        'https://getpublicorganizations-omrwo3ykaa-uc.a.run.app'
      ),
    []
  )
  const getPrivateOrgs = useMemo(
    () =>
      httpsCallableFromURL<{}, Org[]>(
        fns,
        'https://getuserorganizations-omrwo3ykaa-uc.a.run.app'
      ),
    []
  )
          ...
          o.id === orgId
            ? {
                ...
                members: Object.fromEntries(
                  Object.entries(o.members).filter(([id]) => id !== userId)
                ),
              }
            : o
        )
  ...
  const handleLeave = async (orgId: string) => {
    ...
    try {
      ...
      if (original) {
        setOrgsData(prev =>
          prev.map(o => (o.id === orgId ? { ...original, joined: true } : o))
        )
        ...
        <div className="p-8">
          <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-8 mb-8">
            ...
            <Button
              ...
              className="shadow-lg h-10 px-4 py-2 text-base"
            >
                ...
                >
                  ...
                  <Badge variant="secondary" className="ml-2 text-xs">
                    {{
                      ...
                      private: orgsData.filter(o => o.isPrivate && o.joined)
                        .length,
                    }[f]}
                  </Badge>
                </Button>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8">
            {orgs.map(o => {
              ...
              const gradientClass =
                gradients[
                  Math.abs(
                    o.id.split('').reduce((a, b) => a + b.charCodeAt(0), 0)
                  ) % gradients.length
                ]
