...
// import { Button } from "@/components/ui/button" // ❌ not needed anymore
import {
  ChevronDown,
  ChevronRight,
  FileText,
...
function titleFromContentJSON(jsonStr: string): string | null {
  try {
    ...
    for (const block of parsed) {
      // Common BlockNote structure: block.content[].text OR block.props?.text
...
/* ------------------------------ Loader ------------------------------ */
/**
 * Loads ALL notes for an org from RTDB at organizations/{orgId}/notes.
 * Each note may contain fields like { title?, name?, ownerId?, updatedAt?, createdAt?, content? }.
 * If no title is found, we try to derive it from content (BlockNote JSON string).
 */
async function fetchOrgNotes(orgId: string): Promise<NoteListItem[]> {
  ...
  // newest first (updatedAt fallback to createdAt)
...
type NotesBarProps = {
  orgId?: string
  /** Called when user chooses a note (click row). */
...
export default function NotesBar({ orgId, onOpenNote }: NotesBarProps) {
  ...
  async function load() {
    ...
    setNotes([])
    if (!orgId) return
    // Optional auth check (mirrors your save/load functions’ behavior)
    ...
    setLoading(true)
    try {
      const list = await fetchOrgNotes(orgId)
      setNotes(list)
      setExpanded(true)
    } catch (e: any) {
      console.error("[NotesBar] load error:", e)
            ...
            <div className="p-4">
              {!orgId ? (
                ...
                <div className="space-y-2">
                  {/* Single “All Notes” folder (mirrors Quizzes section style) */}
                  <button
