...
function tryLinePairs(text: string) {
  ...
  for (const line of lines) {
    ...
    if (m) { const front = tidy(m[1]), back = tidy(m[2]); if (front && back) out.push({ front, back }); }
  }
  return out.length ? out : null;
}
...
interface FlashcardGeneratorProps { initialText: string; }

export default function FlashcardGenerator({ initialText }: FlashcardGeneratorProps) {
  ...
  function nextCard() { setFlipped(false); setCurrent(prev => (prev + 1) % flashcards.length); }
  function prevCard() { setFlipped(false); setCurrent(prev => (prev - 1 + flashcards.length) % flashcards.length); }
  function reset() { setFlipped(false); setCurrent(0); setFlashcards([]); }

  if (loading) return <div className="flex justify-center items-center h-52"><Loader2 className="h-6 w-6 animate-spin" /></div>;
  if (!flashcards.length) return <div>No flashcards generated.</div>;

  return (
    <Card className="w-96 p-4 flex flex-col items-center gap-4">
      {/* Heading */}
      <CardHeader className="w-full">
        <CardTitle className="text-lg font-semibold">Generate Flashcards</CardTitle>
      </CardHeader>
      ...
      >
        ...
        >
          ...
          <Card className="absolute inset-0 [backface-visibility:hidden] flex items-center justify-center text-center">
            ...
            <CardContent>
              <p className="text-lg overflow-auto break-words">{flashcards[current].front}</p>
            </CardContent>
          </Card>
          ...
          <Card className="absolute inset-0 [backface-visibility:hidden] rotate-y-180 flex items-center justify-center text-center">
            ...
            <CardContent>
              <p className="text-lg overflow-auto break-words">{flashcards[current].back}</p>
            </CardContent>
          </Card>
        </div>
      </div>
      ...
      <div className="flex gap-6 items-center">
        <Button onClick={prevCard} variant="outline"><ChevronLeft className="h-5 w-5" /></Button>
        <span>{current + 1} / {flashcards.length}</span>
        <Button onClick={nextCard} variant="outline"><ChevronRight className="h-5 w-5" /></Button>
        <Button variant="destructive" onClick={reset}><Trash2 className="h-4 w-4" /></Button>
      </div>
    </Card>
  );
